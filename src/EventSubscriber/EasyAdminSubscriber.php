...
class EasyAdminSubscriber implements EventSubscriberInterface
{
    ...
    function setAutomatiqueSerialSlug(GenericEvent $event)
    {
        ...
        if($entity instanceof Gift || $entity instanceof Avatar)
        {
            // $url = $this->service->saveToDisk($entity->getPicture());
            // $entity->setPicture($url);

            $picture = explode("/",$entity->getPicture());
            $entity->setPicture($picture[4]);
            return;
        }   
